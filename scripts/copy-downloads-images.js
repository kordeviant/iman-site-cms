...
// Copy only image files (jpg, jpeg, png, webp, gif) from downloads subfolders
// to site/static/downloads-images preserving flat names (prefix with folder)
// and produce site/static/downloads-index.json with list of relative URLs.
...
entries.forEach(dirent => {
  ...
  files.forEach(f => {
    ...
    if (!imageExt.has(ext)) return; // skip videos and others
    const src = path.join(folder, f.name);
    const destName = sanitizeFilename(dirent.name + '---' + f.name);
    const dest = path.join(outDir, destName);
    fs.copyFileSync(src, dest);
    // URL relative to site root (Hugo will serve site/static as /)
