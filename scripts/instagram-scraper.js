...
class InstagramScraper {
  ...
  ensureDirectories() {
    [this.userDataDir, this.baseDir, this.contentDir, this.imagesDir].forEach(
      (dir) => {
  ...
  // Check if products already exist to avoid re-scraping
  checkExistingProducts() {
    try {
      const files = fs.readdirSync(this.contentDir);
      const productFiles = files.filter(
        (file) => file.startsWith("instagram-") && file.endsWith(".md")
      );

      if (productFiles.length > 0) {
        console.log(
          `📦 Found ${productFiles.length} existing Instagram products`
        );
        return true;
      }
      return false;
  ...
  async downloadProfileImage(profileInfo) {
    ...
    try {
      ...
      const success = await this.downloadMedia(
      ...
      if (success) {
        ...
        const logoBackupPath = path.join(
          __dirname,
          "../site/static/img/logo-backup.svg"
        );
        try {
          ...
          console.log(
            "⚠️ Could not replace logo, but profile image downloaded successfully"
          );
        }
      ...
      try {
        ...
        const request = https.get(url, (response) => {
          if (response.statusCode === 200) {
            ...
            console.log(
              `❌ Failed to download ${type} (${response.statusCode}): ${filename}`
            );
            file.close();
  ...
  async createProductPage(post, mediaFiles) {
    try {
      ...
      // Use a placeholder image if no media is available
      const primaryMedia =
        mediaFiles.images.length > 0
          ? mediaFiles.images[0].filename
          : mediaFiles.videos.length > 0
          ? mediaFiles.videos[0].filename
          : "placeholder.jpg";

      // Gallery images for Hugo gallery (only if media exists)
      const allGallery = [...mediaFiles.images, ...mediaFiles.videos].map(
        (media) => `/img/products/${media.filename}`
      );
      ...
      // Create a meaningful title from the description or use a default
      const title = post.description
        ? post.description.substring(0, 50).replace(/\n/g, " ").trim() +
...
description: "${escapeYaml(
        post.description ||
          "Beautiful jewelry piece from our Instagram collection"
      )}"
...
${
  post.description ||
  "This beautiful jewelry piece is part of our exclusive collection. Contact us for more details and pricing."
}
...
- **Source**: [Instagram](${post.url})
- **Images Available**: ${mediaFiles.images.length}
- **Videos Available**: ${mediaFiles.videos.length}
...
*Contact us for pricing and availability of this unique piece.*
`;
  ...
  async updateInstagramProfile(profileInfo) {
    try {
      const profilePath = path.join(
        __dirname,
        "../site/content/data/instagram-profile.json"
      );
      const profileData = {
        username: profileInfo.username,
        displayName: profileInfo.displayName,
        bio: profileInfo.bio,
        profileImage: profileInfo.profileImageFile
          ? `/img/products/${profileInfo.profileImageFile}`
          : null,
        stats: {
  ...
  async syncPostsWithProductsUsingModals(postElements, profileUsername) {
    const results = { total: postElements.length, successful: 0, failed: 0 };

    // Clean existing Instagram products
    await this.cleanExistingInstagramProducts();

    console.log(
      `🔄 Processing ${postElements.length} posts sequentially using modals...`
    );

    // Process each post element sequentially
    for (let i = 0; i < postElements.length; i++) {
      ...
      try {
        ...
        // Download media files from modal
        const { postId, mediaFiles } = await this.downloadPostMediaFromModal(
          postElement
        );

        if (!postId) {
          console.log("⚠️ Could not extract post ID, skipping...");
          results.failed++;
          continue;
        }

        // Create post object for product page creation
        const post = {
          id: postId,
  ...
  async scrapeAndCreateProducts(instagramUrl, options = {}) {
    ...
    // Check if products already exist and skip if not forcing re-scrape
    if (!options.force && this.checkExistingProducts()) {
      console.log("⏭️ Products already exist. Skipping scraping.");
      console.log(
        "💡 Use --force option or run clean-products script first if you want to re-scrape."
      );
      return results;
    }
    ...
    try {
      ...
      if (profile) {
        ...
        console.log(
          `📊 Posts: ${profile.postsCount}, Followers: ${profile.followersCount}, Following: ${profile.followingCount}`
        );
      ...
      // Get all post elements for modal-based processing
      const postElements = await this.scrapePostElements(options.limit || 10);
      results.total = postElements.length;

      console.log(
        `📱 Found ${postElements.length} posts to process using modals`
      );

      // Process posts using modal approach
      const modalResults = await this.syncPostsWithProductsUsingModals(
        postElements,
        profile?.username
      );
      results.successful = modalResults.successful;
      results.failed = modalResults.failed;
    } catch (error) {
    ...
    try {
      ...
      const profileInfo = await this.page.evaluate(() => {
        ...
        return {
          ...
          bio:
            getTextContent('div[data-testid="user-bio"]') ||
            getTextContent("div.-vDIg span"),
          profileImageUrl: profileImageUrl,
          postsCount: getTextContent('a[href*="/"] span').replace(/,/g, ""),
          followersCount:
            getTextContent('a[href*="/followers/"] span') ||
            getTextContent('a[href*="/followers/"] title'),
      ...
      if (profileInfo.profileImageUrl) {
        profileInfo.profileImageFile = await this.downloadProfileImage(
          profileInfo
        );
      }
  ...
  async scrapePostElements(limit = 10) {
    try {
      console.log(`📱 Finding ${limit} post elements...`);

      // Scroll to load posts
      await this.scrollToLoadPosts();

      // Get post elements as DOM handles
      const postElements = await this.page.$$('a[href*="/p/"]');

      console.log(
        `✅ Found ${Math.min(postElements.length, limit)} post elements`
      );
      return postElements.slice(0, limit);
    } catch (error) {
      console.error("❌ Error finding post elements:", error.message);
      return [];
    }
  }
    ...
    try {
      ...
      const posts = await this.page.evaluate((limit) => {
        const postElements = document.querySelectorAll(
          'article div[role="button"]:has(img), a[href*="/p/"]'
        );
        const posts = [];
  ...
  async downloadPostMediaFromModal(postElement) {
    try {
      const postId = await this.page.evaluate((el) => {
        const link = el.href || el.closest("a")?.href;
        return link ? link.split("/p/")[1].split("/")[0] : null;
      }, postElement);

      console.log(`📸 Opening modal for post: ${postId}`);

      // Click on the post to open modal
      ...
      // Get media URLs from modal
      const mediaUrls = await this.page.evaluate(() => {
        const modal = document.querySelector('[role="dialog"]');
        if (!modal) return { images: [], videos: [] };
        ...
        const videos = Array.from(modal.querySelectorAll("video")).map(
          (video) => ({
            url: video.src,
      ...
      for (let i = 0; i < mediaUrls.images.length; i++) {
        ...
        if (img.url && !img.url.includes("profile") && img.width > 100) {
          const filename = `${postId}-img-${i + 1}.jpg`;
      ...
      for (let i = 0; i < mediaUrls.videos.length; i++) {
        ...
        if (video.url) {
          const filename = `${postId}-video-${i + 1}.mp4`;
          const success = await this.downloadMedia(
            video.url,
            filename,
            "video"
      ...
      return { postId, mediaFiles };
    } catch (error) {
      console.error(`❌ Error downloading media from modal:`, error.message);
      ...
      return { postId: null, mediaFiles: { images: [], videos: [] } };
    }
  }
}
